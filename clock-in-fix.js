...
(function () {
  ...
  function startSandbox() {
    ...
    function run() {
      ...
      try { if (sbCtl && sbCtl.stop) sbCtl.stop(); } catch (_) {}
      try {
        ...
        setTimeout(function () {
          ...
          if (typeof window._ltEnsureEasySeat === "function") { try { window._ltEnsureEasySeat(); } catch (_) {} }
        }, 100);
        setTimeout(function () {
          if (typeof window._ltEnsureEasySeat === "function") { try { window._ltEnsureEasySeat(); } catch (_) {} }
        }, 400);
  ...
  function startElectrical(opts) {
    ...
    if (lab && lab.start) { try { lab.start(root, opts || {}); } catch (_) {} }
    setTimeout(function () {
  ...
  function bindShopFloor(id) {
    ...
    if (!hub) return;
    hub.onclick = function (e) { if (e) e.preventDefault(); goHub(); };
  }
  function rescuePlay(m) {
    ...
    if (m === "quiz" && window.QuizArena && window.QuizArena.start) {
      show("quiz"); try { window.QuizArena.start(document.getElementById("quiz-root")); } catch (_) {} return;
    }
    if (m === "minisplit" && window.MiniSplit && window.MiniSplit.start) {
      show("minisplit"); try { window.MiniSplit.start(document.getElementById("minisplit-root")); } catch (_) {} return;
    }
    if (m === "service") {
      show("service");
      var host = document.getElementById("screen-service") || document.getElementById("svc-choices");
      if (window.ServiceCalls && window.ServiceCalls.start) { try { window.ServiceCalls.start(host, { onHub: goHub }); } catch (_) {} }
      bindShopFloor("btn-svc-hub"); return;
    }
    if (m === "epa608" && window.Epa608 && window.Epa608.start) {
      show("epa608"); try { window.Epa608.start(document.getElementById("epa608-root")); } catch (_) {} return;
    }
    if (m === "commandments" && window.Commandments && window.Commandments.start) {
      show("commandments"); try { window.Commandments.start(document.getElementById("commandments-root")); } catch (_) {} return;
    }
    if (m === "rapture") { show("rapture"); bindShopFloor("btn-rapture-hub"); return; }
    if (m === "boardcodes") {
      goHub();
      setTimeout(function () {
    ...
    if (m === "truck-pouch") {
      ...
      if (tp && TP && TP.start) { try { TP.start(tp); } catch (_) {} }
      return;
    }
    show(m);
  }
  function play(m) {
    if (!m) return;
    if (m === "hub") return goHub();
    if (m === "boardcodes" || m === "service" || m === "defusal" || m === "electrical" || m === "elguide" || m === "sandbox") {
      rescuePlay(m); return;
    }
