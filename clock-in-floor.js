...
(function () {
  function show(id) {
    ...
    var labs = {
      ...
      epa608: 1, film: 1, shoplabs: 1, "truck-pouch": 1, ohm: 1, ohms: 1, "ohms-law": 1,
  ...
  var sbCtl = null;
  var sbScripts = ["sandbox.js?v=148", "sandbox-hook.js?v=14"];
  var sbLoading = false;
  ...
  /* ?play=1 skips locker / Clock In splash and lands on the shop floor (optional #mode). */
  function ltPlayQuery() {
    try {
      var q = new URLSearchParams(location.search || "");
      if (q.get("play") !== "1") return;
      function land() {
                try { if (typeof window.ltGoHub === "function") window.ltGoHub(); else goHub(); } catch (_) {} try {
          var title = document.getElementById("screen-title");
          if (title) {
            title.classList.remove("active");
            title.classList.remove("screen-on");
        ...
        if (hash && typeof play === "function") {
          try { play(hash); } catch (_) {}
        }
      }
      land();
      setTimeout(land, 50);
  ...
  if (document.readyState === "loading") document.addEventListener("DOMContentLoaded", function () { bind(); ltPlayQuery(); });
  else { bind(); ltPlayQuery(); }
